...
namespace Assets.Models.Enemies.Classes
{
    public class Bowman : Enemy
    {
        ...
        public Bowman(float maxHealth, float health, float attackDmg, float attackSpeed, float attackTime, float attackRange, int exp, int gold)
        {
            go = GameObject.Instantiate(this.GetClassGameObject());
            SetStats(go);
